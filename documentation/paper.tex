...
\newcommand{\bb}{$\null$\vspace{-5pt}\\\\}
\begin{document}
\title{\Large{My Endurance Trainer: A Wearable Health Monitor for Endurance Runners}}
\author{\IEEEauthorblockN{Chih-Yu Andy Liu\ \ \ \ }
\and
\IEEEauthorblockN{\ \ \ \ Tracy Lu\ \ \ \ \ \ \ \ }
\and
\IEEEauthorblockN{Felix Wong}
...
\begin{abstract}
We build a portable bracelet that helps endurance runners.
\end{abstract}
...
$$\{(t,h,p,\theta)=({timestep, heart rate, pedometer, temperature})\},$$
where
$$
\begin{cases}
 t\in [0,8640],\\
...
%triangles kidney donor/receiver
